...
async function dashboardStats() {
  const cookieStore = await cookies();
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000/api'}/clients/stats`, { headers: { cookie: cookieStore.toString() }, cache: 'no-store' });
    if (!response.ok) throw new Error();
    const data = await response.json();
    return [['Clients',String(data.clients)],['Pending posts',String(data.pendingPosts)],['Scheduled today',String(data.scheduledToday)],['Published today',String(data.publishedToday)],['Failed',String(data.failed)]];
  } catch { return [['Clients','0'],['Pending posts','0'],['Scheduled today','0'],['Published today','0'],['Failed','0']]; }
}
...
export default async function DashboardPage() {
  ...
  return <main className="dashboard">
    <aside className="sidebar"><div className="brand">NextServe AI</div><nav className="nav"><Link className="active" href="/dashboard">Overview</Link><Link href="/clients">Clients</Link><Link href="#">Content studio</Link><Link href="#">Calendar</Link><Link href="#">Analytics</Link></nav><div className="userbox"><strong>{user.name}</strong><div style={{color:'#93a69e',fontSize:13,marginTop:5}}>{user.email}</div></div></aside>
